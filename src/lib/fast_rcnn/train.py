...
class SolverWrapper(object):
    ...
    def __init__(self, solver_prototxt, roidb, output_dir,
        ...
        if cfg.TRAIN.BBOX_REG:
            ...
            tmp_means = np.tile(
                np.array(cfg.TRAIN.INCLINED_RECT_NORMALIZE_MEANS), (num_reg_classes, 1))
            tmp_stds = np.tile(
                np.array(cfg.TRAIN.INCLINED_RECT_NORMALIZE_STDS), (num_reg_classes, 1))
            self.inclined_rect_means, self.inclined_rect_stds = tmp_means.ravel(), tmp_stds.ravel()
    ...
    def snapshot(self):
        ...
        scale_inclined_params_faster_rcnn = (cfg.TRAIN.BBOX_REG and
                             cfg.TRAIN.BBOX_NORMALIZE_TARGETS and
                             net.params.has_key('inclined_rect_pred'))
        ...
        if scale_inclined_params_faster_rcnn:
            # save original values
            inclined_orig_0 = net.params['inclined_rect_pred'][0].data.copy()
            inclined_orig_1 = net.params['inclined_rect_pred'][1].data.copy()

            # scale and shift with bbox reg unnormalization; then save snapshot
            net.params['inclined_rect_pred'][0].data[...] = \
                (net.params['inclined_rect_pred'][0].data *
                 self.inclined_rect_stds[:, np.newaxis])
            net.params['inclined_rect_pred'][1].data[...] = \
                (net.params['inclined_rect_pred'][1].data *
                 self.inclined_rect_stds + self.inclined_rect_means)
        ...
        if scale_inclined_params_faster_rcnn:
            # restore net to original state
            net.params['inclined_rect_pred'][0].data[...] = inclined_orig_0
            net.params['inclined_rect_pred'][1].data[...] = inclined_orig_1
        if scale_bbox_params_rfcn:
            # restore net to original state
